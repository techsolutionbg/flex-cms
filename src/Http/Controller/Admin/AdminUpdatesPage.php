...
{
    public function __construct(private AuthenticationInterface $authentication, private SettingRepository $settings, private PlatformVersionRegistry $versions, private ViewRendererInterface $views, private ViteAssetManager $assets) {}
    ...
    public function render(string $csrfToken, array $history, ?string $notice = null, ?string $error = null, ?array $inspection = null): string
    {
        ...
        $bootstrap = ['page' => 'updates', 'csrfToken' => $csrfToken, 'sidebarWidth' => $user instanceof AuthenticatedUser ? $this->settings->sidebarWidthForUser($user->id) : 248, 'version' => $this->versions->current()->value, 'history' => array_reverse($history), 'notice' => $notice, 'error' => $error, 'inspection' => $inspection];
