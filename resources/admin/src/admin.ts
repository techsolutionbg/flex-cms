...
function pluginPageFieldsMarkup(): string {
  return `<template x-for="field in pageFields" :key="field.plugin + '.' + field.id"><label x-show="field.type !== 'checkbox'"><span class="field-label" x-text="field.label"></span><template x-if="field.type === 'textarea'"><textarea class="form-control" x-model="pluginFields[field.plugin][field.id]" :maxlength="1000"></textarea></template><template x-if="field.type === 'text'"><input class="form-control" type="text" x-model="pluginFields[field.plugin][field.id]" :maxlength="190"></template><span class="field-hint-text" x-show="field.hint" x-text="field.hint"></span></label><label class="checkbox-field" x-show="field.type === 'checkbox'"><input type="checkbox" x-model="pluginFields[field.plugin][field.id]"><span x-text="field.label"></span><span class="field-hint-text" x-show="field.hint" x-text="field.hint"></span></label></template>`
}
