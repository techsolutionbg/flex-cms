...
import { dataTableMarkup, registerDataTable } from "@/components/data-table"
...
function createAdminState(initial: Bootstrap) {
  const state = {
    ...
    navigate(url: string, pushHistory = true) { $.get(url, (markup: string) => { const next = new DOMParser().parseFromString(markup, "text/html").getElementById("flex-admin-bootstrap"); if (!next) return; this.applyBootstrap(JSON.parse(next.textContent ?? "{}") as Bootstrap); if (pushHistory) window.history.pushState({}, "", url); window.scrollTo({ top: 0, behavior: "smooth" }) }, "html").fail(() => { this.error = "Страницата не можа да бъде заредена." }) },
