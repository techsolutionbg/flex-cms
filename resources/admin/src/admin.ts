    ...
    <div class="admin-shell sidebar-enhanced" x-data="adminApp" :class="{ 'sidebar-collapsed': collapsed, 'sidebar-mobile-open': mobileOpen }" :style="\`--sidebar-width: \${width}px\`"><div class="admin-loading-bar" x-show="loading" x-cloak></div>
      ...
      <div class="admin-main">
        ...
        <main class="admin-content">
          ...
          <template x-if="page === 'plugin-detail'"><section>${breadcrumbsMarkup("pluginDetail?.name ?? 'Разширение'", [{ label: "Разширения", href: "/admin/plugins" }])}<div class="page-heading-row"><div><h1 x-text="pluginDetail?.name"></h1><p class="lead" x-text="pluginDetail?.description || 'Подробности и lifecycle управление на разширението.'"></p></div><div class="page-heading-actions"><button class="button secondary" type="button" @click="navigate('/admin/plugins')">Назад към разширенията</button><button class="button secondary" type="button" x-show="pluginDetail?.status === 'installed' || pluginDetail?.status === 'inactive'" @click="pluginAction('activate', pluginDetail)">Активирай</button><button class="button secondary" type="button" x-show="pluginDetail?.status === 'active'" @click="pluginAction('deactivate', pluginDetail)">Деактивирай</button></div></div><div class="plugin-detail-grid"><section class="content-card"><header>${sectionHeader("Основна информация", "plugin-detail-info")}</header><div class="section-body plugin-detail-list"><div><span>ID</span><strong x-text="pluginDetail?.id"></strong></div><div><span>Версия</span><strong x-text="pluginDetail?.version"></strong></div><div><span>Entrypoint</span><strong x-text="pluginDetail?.entrypoint"></strong></div><div><span>Път</span><strong x-text="pluginDetail?.path"></strong></div><div><span>Статус</span><strong x-text="pluginDetail?.status"></strong></div></div></section><section class="content-card"><header>${sectionHeader("Manifest", "plugin-detail-manifest")}</header><div class="section-body"><pre class="plugin-manifest" x-text="JSON.stringify(pluginDetail?.manifest || {}, null, 2)"></pre></div></section></div><div class="notice error" x-show="pluginDetail?.last_error" x-text="pluginDetail?.last_error"></div></section></template>
