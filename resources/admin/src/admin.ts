...
type PluginRecord = {
  ...
  entrypoint: string
  path: string
  status: "discovered" | "installed" | "inactive" | "active"
  installed_at: string | null
  activated_at: string | null
}

type Bootstrap = {
  page: "dashboard" | "updates" | "profile" | "pages" | "pages-create" | "pages-edit" | "pages-settings" | "plugins"
  csrfToken: string
  sidebarWidth: number
    ...
    <div class="admin-shell sidebar-enhanced" x-data="adminApp" :class="{ 'sidebar-collapsed': collapsed, 'sidebar-mobile-open': mobileOpen }" :style="\`--sidebar-width: \${width}px\`"><div class="admin-loading-bar" x-show="loading" x-cloak></div>
      <aside id="admin-sidebar" class="admin-sidebar" :aria-hidden="isMobile && !mobileOpen">
        ...
        <nav aria-label="Административна навигация"><ul class="sidebar-nav-list">
          ...
          <li><a class="sidebar-link" href="/admin/plugins" :class="{ 'is-active': page === 'plugins' }"><svg class="sidebar-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3v4M16 3v4M5 8h14v11a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V8Zm3 4h8M8 16h5" /></svg><span class="sidebar-link-label">Разширения</span></a></li>
      ...
      <div class="admin-main">
        ...
        <main class="admin-content">
          ...
          <template x-if="page === 'plugins'"><section>${breadcrumbsMarkup("'Разширения'")}<div class="page-heading-row"><div><h1>Разширения</h1><p class="lead">Управлявайте разширенията, които добавят функционалност към Flex CMS.</p></div></div><div class="notice error" x-show="error" x-text="error"></div><div class="pages-table table-wrapper"><table><thead><tr><th>Име</th><th>ID</th><th>Версия</th><th>Статус</th><th>Действия</th></tr></thead><tbody><template x-for="plugin in plugins" :key="plugin.id"><tr><td><strong x-text="plugin.name"></strong><small class="table-cell-secondary" x-text="plugin.description"></small></td><td x-text="plugin.id"></td><td x-text="plugin.version"></td><td><span class="status-badge" x-text="plugin.status === 'active' ? 'Активен' : plugin.status === 'installed' || plugin.status === 'inactive' ? 'Неактивен' : 'Открит'"></span></td><td class="table-actions"><button class="button secondary" type="button" x-show="plugin.status === 'discovered'" :disabled="pluginBusy === plugin.id" @click="pluginAction('install', plugin)" x-text="pluginBusy === plugin.id ? 'Изпълнение…' : 'Инсталирай'"></button><button class="button secondary" type="button" x-show="plugin.status === 'installed' || plugin.status === 'inactive'" :disabled="pluginBusy === plugin.id" @click="pluginAction('activate', plugin)" x-text="pluginBusy === plugin.id ? 'Изпълнение…' : 'Активирай'"></button><button class="button secondary" type="button" x-show="plugin.status === 'active'" :disabled="pluginBusy === plugin.id" @click="pluginAction('deactivate', plugin)" x-text="pluginBusy === plugin.id ? 'Изпълнение…' : 'Деактивирай'"></button><button class="button danger" type="button" x-show="plugin.status === 'inactive' || plugin.status === 'installed'" :disabled="pluginBusy === plugin.id" @click="pluginAction('uninstall', plugin)">Премахни</button></td></tr></template><tr x-show="plugins.length === 0"><td colspan="5" class="empty-state">Няма открити плъгини.</td></tr></tbody></table></div></section></template>
...
function createAdminState(initial: Bootstrap) {
  ...
  const state = {
    ...
    form: { title: "", slug: "", content: "", parentId: "" as string | number, status: "draft" as "draft" | "published" }, pageSettings: { seo_title: "", meta_description: "", canonical_url: "", template: "default", menu_order: 0, use_parent_slugs: false, no_index: false, show_in_navigation: true, show_in_sitemap: true } as PageSettings,
  ...
  state.pageTitle = ({ dashboard: "Табло", updates: "Обновявания", pages: "Страници", "pages-create": "Създаване на страница", "pages-edit": "Редактиране на страница", "pages-settings": "Настройки на страница", profile: "Профил", plugins: "Разширения" } as Record<string, string>)[initial.page]
  return state
}
