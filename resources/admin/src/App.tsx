...
function HistoryTable({
      ...
      [...history].sort((left, right) => {
        ...
        const result =
          sortKey === "date"
            ...
            : leftValue.localeCompare(rightValue, "bg", {
                numeric: true,
                sensitivity: "base",
              })
  ...
  const pageRows = sortedHistory.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
  )
  const firstItem =
    sortedHistory.length === 0 ? 0 : (currentPage - 1) * pageSize + 1
  ...
  return (
    <div className="space-y-4">
      <Table>
        <TableHeader>
          <TableRow>
              ...
              return (
                ...
                >
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className="m-0 h-7 w-full justify-start rounded-none px-2 font-medium"
            ...
            <TableHead className="h-8 border border-border px-2 py-1 text-right">
              Действия
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {pageRows.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={6}
                className="h-16 py-1 text-center text-muted-foreground"
              >
                Все още няма записи.
              </TableCell>
            </TableRow>
      ...
      <div className="flex items-center justify-center gap-3 text-sm whitespace-nowrap text-muted-foreground">
        <span>
          Показване {firstItem}–{lastItem} от {sortedHistory.length}
        </span>
