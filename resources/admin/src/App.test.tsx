...
describe("admin application shell", () => {
  it("renders the dashboard navigation and content", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Административен панел" })
    ).toBeInTheDocument()
    expect(
    ...
    expect(screen.getByRole("link", { name: /Табло/ })).toHaveClass("is-active")
    expect(screen.getByRole("link", { name: "Обновявания" })).not.toHaveClass(
      "is-active"
    )
  })
})
